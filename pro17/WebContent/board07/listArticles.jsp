...
<body>
	...
	<c:choose>
		...
		<c:when test="${!empty articlesList}" >
			<c:forEach var="article" items="${aritlcesList}" varStatus="articleNum">
				<tr align="center">
					...
					<td width='left' width="35%">
						...
						<c:choose>
							<c:when test='${article.level > 1}'>
								<forEach begin="1" end="${article.level}" step="1">
							...
							<c:otherwise>
								<a class="cls1" href="${contextPath}/board/viewArticle.do?articleNO=${article.articleNO}">${aritcle.title}</a>
							</c:otherwise>
						</c:choose>
					</td>
		...
		<c:if test="${totArticles != null }">
			<c:choose>
				<c:when test="${totArticles > 100 }"> <!-- 글 개수가 100 초과인 경우 -->
					<forEach var="page" begin="1" end="10" step="1">
